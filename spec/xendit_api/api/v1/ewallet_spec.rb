...
require 'xendit_api/errors/v1/ovo'
require 'xendit_api/client'

RSpec.describe XenditApi::Api::V1::Ewallet do
  let(:client) { XenditApi::Client.new('***REMOVED***') }
  ...
  describe '#post' do
    context 'with valid params' do
      ...
      it 'returns success response' do
        VCR.use_cassette('xendit/ewallet/v1/ovo/success') do
    ...
    context 'with invalid params' do
      ...
      it 'raise error phone number not registered' do
        VCR.use_cassette('xendit/ewallet/v1/ovo/errors_invalid_phone_number') do
          expect do
            ewallet_api = described_class.new(client)
      ...
      it 'raise error channel not activated' do
        VCR.use_cassette('xendit/ewallet/v1/ovo/errors_channel_not_activated') do
          expect do
            ewallet_api = described_class.new(client)
            params = { reference_id: 'fa37759f-6bb0-4b7f-9701-2b6f43af01c9', amount: 10_100, mobile_number: '+6282310202012' }
            ewallet_api.post(params: params, channel_code: 'ID_OVO')
          end.to raise_error(XenditApi::Errors::V1::OVO::ChannelNotActivated)
        end
      end

      it 'raise error channel unavailable' do
        VCR.use_cassette('xendit/ewallet/v1/ovo/errors_channel_unvailable') do
          expect do
            ewallet_api = described_class.new(client)
            params = { reference_id: '7f156109-5f7a-479a-84db-f59c5ab38766', amount: 10_101, mobile_number: '+6282310202012' }
            ewallet_api.post(params: params, channel_code: 'ID_OVO')
          end.to raise_error(XenditApi::Errors::V1::OVO::ChannelUnavailable)
        end
      end

      it 'raise error server error' do
        VCR.use_cassette('xendit/ewallet/v1/ovo/errors_server_error') do
          expect do
            ewallet_api = described_class.new(client)
      ...
      it 'raises error XenditApi::Errors::OVO::DuplicateError' do
        VCR.use_cassette('xendit/ewallet/v1/ovo/errors_duplicate_payment') do
          expect do
            ewallet_api = described_class.new(client)
            params = { reference_id: 'eacd7788-8864-421c-a39c-9c59c3ee875c', amount: 4000, mobile_number: '+6282310202012' }
            ewallet_api.post(params: params, channel_code: 'ID_OVO')
          end.to raise_error(XenditApi::Errors::V1::OVO::DuplicateError, 'There is already a charge request with the same reference_id.')
        end
      end

      it 'raises error XenditApi::Errors::UnknownError' do
        VCR.use_cassette('xendit/ewallet/v1/ovo/errors_unknown_error') do
          expect do
            ewallet_api = described_class.new(client)
  ...
  describe '#get' do
    context 'when payment was complete' do
      it 'returns expected response' do
        VCR.use_cassette('xendit/ewallet/v1/ovo/get_complete_payment') do
          ewallet_api = described_class.new(client)
    ...
    context 'when payment not found' do
      it 'returns error response' do
        VCR.use_cassette('xendit/ewallet/v1/ovo/get_payment_not_found') do
          expect do
            ewallet_api = described_class.new(client)
            random_uuid = 'ewc_d351c488-fd5c-4a41-975f-f94614f7628f'
            ewallet_api.get(id: random_uuid)
          end.to raise_error(XenditApi::Errors::V1::OVO::DataNotFound, 'Charge request not found')
        end
      end
    end
  end
